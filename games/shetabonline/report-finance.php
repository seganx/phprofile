...
$revenues = array();
$db->query("SELECT DAY(`timestamp`) as day, COUNT(*) as count, SUM(`price`) as total FROM `purchases` WHERE UNIX_TIMESTAMP(timestamp) > {$start_time} GROUP BY DATE_FORMAT(timestamp, '%Y%m%d') ORDER BY DATE_FORMAT(timestamp, '%Y%m%d') DESC");
if ($db->has_result())
{
...
$skus = array();
$db->query("SELECT `sku`, COUNT(*) as count, SUM(`price`) as total FROM `purchases` WHERE UNIX_TIMESTAMP(timestamp) > {$start_time} GROUP BY `sku` ORDER BY count DESC");
if ($db->has_result())
{
	while($r = $db->result->fetch_assoc())
	{
...
$markets = array();
$db->query("SELECT `market`, COUNT(*) as count, SUM(`price`) as total FROM `purchases` WHERE UNIX_TIMESTAMP(timestamp) > {$start_time} GROUP BY `market` ORDER BY `total` DESC");
if ($db->has_result())
{
	while($r = $db->result->fetch_assoc())
	{
