...
function svg_ltv_line(array $curve, int $cohortSize): string {
    ...
    $maxX = max($days);
    $maxY = 0.0;
    foreach ($curve as $d => $v) { $maxY = max($maxY, (float)$v['avg_ltv']); }
    if ($maxY <= 0) $maxY = 1;
...
function svg_market_bars(array $markets): string {
    ...
    usort($markets, function($a,$b){ return ($b['revenue'] <=> $a['revenue']); });

    $labels = array_map(fn($m)=>$m['market'] === '' ? '(unknown)' : $m['market'], $markets);
    $values = array_map(fn($m)=> (float)$m['revenue'], $markets);
    $n = count($values);
...
$totalRevenue = 0.0;
$payingUsers = 0;
$curve = [];           // day_since_join => ['rev_day','rev_cum','avg_ltv']
$maxDay = 0;
$purchasesWindow = ['min' => null, 'max' => null];
    ...
    if ($cohortSize > 0) {
        ...
        for ($d = 0; $d <= $maxDay; $d++) {
            ...
            $curve[$d] = [
                'rev_day' => $dayRev,
                'rev_cum' => $cum,
                'avg_ltv' => $cohortSize > 0 ? ($cum / $cohortSize) : 0.0,
            ];
        }
